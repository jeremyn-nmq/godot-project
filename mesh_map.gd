...
func generate_terrain():
	...
	for z in range(localData.size() + 1):
		for x in range(localData[0].size() + 1):
			var y = 0
			surftool.add_vertex(Vector3(x,y,z))
			if(localData[z-1][x-1] == 1):
				draw_box(Vector3(x,y+0.5,z))
			if(localData[z-1][x-1] == 2):
				draw_ocupied_lot(Vector3(x,y+0.5,z))
			if(localData[z-1][x-1] == 3):
				draw_empty_lot(Vector3(x,y+0.1,z))
			if(localData[z-1][x-1] == 4):
				draw_neareast_empty_lot(Vector3(x,y+0.1,z))	
				target = Vector3(x,y,z)
	
	var zSize = localData.size()
	var xSize = localData[0].size()
	var vert = 0
